...
function makeDb(): Database.Database {
  ...
  const ev = db.prepare(
    `INSERT INTO events (session_id, workspace_id, ts, type, model, input_tokens, output_tokens,
       service_tier, raw_jsonl, dedup_key) VALUES (?,?,?,?,?,?,?,?,?,?)`
  );
  ev.run('sa', WS_A, 1000, 'assistant', 'claude-x', 10, 20, 'standard', '{"a":1}', 'da');
  ev.run('sb', WS_B, 2000, 'assistant', 'claude-x', 99, 99, 'standard', '{"secret":true}', 'db');
  return db;
}
