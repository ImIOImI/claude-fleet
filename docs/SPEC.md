...
- `plan_usage({ window_s?, at? })` → app-wide aggregate: total USD + tokens, `byModel` and `byBackend` breakdowns, latest anchor (`resetAt`, `scope`), and provisional `usedPct` of the plan cap. **No grant required; aggregates only** — the same non-private carve-out as global-error rows; never emits per-workspace rows or transcript detail. Injected via `setPlanUsageHandler` from `ipc.ts` (which alone knows backend kinds), so `mcpServer.ts` needn't import the backend graph.
