...
- **`committee_post(id, message)`** (MCP) / `committee:post` (IPC) → `assertControl('post')` → `backend.committeePost(targetId, text)`. The docker impl is a **transient** broker attach: `LIST` the live sessions (guard **0** = "not attached yet", **>1** = "single-tab expert only" in v1) → `ATTACH` the one session (reattach-retry on the `already attached` race) → `sendInput(text + '\r')` → `DETACH`. Same fire-and-forget path as a human keystroke; no new broker frame. It deliberately does **not** read the session's output stream — replies come back via `collect` from the DB, so output capture never depends on this attach. **Writer conflict:** the broker allows one writer per session, so if a human is viewing the expert (renderer holds the writer) a post is refused `already attached` — a watched expert can't be posted to in v1 (documented limitation; the headless committee case is conflict-free). Mock acks without a broker; local throws (container-only).
...
- The effect functions live in `ipc.ts` (`committeePost`/`committeeCollect`), shared by the IPC channels and the MCP tools (injected via `setCommitteeHandlers`). `Backend.committeePost` is the per-backend broker hook (docker real / mock ack / local throw); collect needs no backend (the events DB is shared). **Verification gap:** the real broker `sendInput` round-trip can't be exercised in the mock/no-container e2e harness — `mcp-server.spec.ts` covers the collect data path against a real DB and the post/collect *authorization* is covered in mock; the docker post round-trip needs a real-container smoke test before relying on it.
