...
The image also bakes two hook artifacts into `/usr/local/lib/claude-fleet/`: **`input-wait-report.sh`** (the hook script — calls `signal_input_wait` via the MCP socket on `PreToolUse[AskUserQuestion]` with `waiting=true`, and on `PostToolUse[AskUserQuestion]`/`Stop`/`UserPromptSubmit` with `waiting=false`) and **`hooks.settings.json`** (a claude settings file that registers `input-wait-report.sh` for those hook events). Every in-container `claude` launch passes `--settings /usr/local/lib/claude-fleet/hooks.settings.json` — a **trusted load** that bypasses the `/hooks` approval gate — so the hook fires without user confirmation. These two files and the `--settings` flag are the sole mechanism behind the waiting-chip indicator; they do not affect the user's own `.claude/settings.json`.

**Phase 2 — session-summary hook (not yet implemented).** The runner's `Stop` hook will write a `session-summary` JSONL event (`{"type":"session-summary","summary":"…","sessionId":"…","model":"…"}`) at the end of each turn. The host ingest path already handles this event type (routes into `session_summaries` and schedules embedding). When the hook lands, it will be registered in `hooks.settings.json` alongside `input-wait-report.sh` and baked into the runner image. Until Phase 2 ships, the `session_summaries` table is populated only by the incremental indexer's own summarization pass.
...
The app uses Electron's default **native menu** (`Menu.setApplicationMenu`). The **Help** submenu carries two items built in `src/main/appMenu.ts` and opened WSL-aware via `openHostPath`: **Open Data Folder** (`app.getPath('userData')`) and **Open Log** (`getLogPath()` → `<userData>/error.log`). These are the primary escape hatches for users who need to locate crash logs or the state DB without knowing the path.
...
-- One row per session; populated by ingesting 'session-summary' JSONL events
-- (Phase 2 — runner Stop hook; see runner image section below) and also by
-- the incremental indexer when a session ends.
CREATE TABLE session_summaries (
  session_id          TEXT PRIMARY KEY,
  workspace_id        TEXT NOT NULL,
  summary             TEXT NOT NULL,
  source_max_event_id INTEGER NOT NULL,  -- max events.id at summary-generation time
  model               TEXT,
...
`logError` is the single write path. File append to `<userData>/error.log` (one JSON line per call) remains the primary, crash-safe output; `recordError` is a best-effort DB sink registered via `setErrorSink` — a wedged DB never breaks crash logging. `LogPayload` carries optional `workspaceId`, `sessionId`, and `level` fields (absent ⇒ `null` / `'error'`), enabling operational diagnostics to be attributed to a workspace or session alongside crash rows.
...
**`session-summary` JSONL event type (Phase 2 contract).** The runner's `Stop` hook (not yet implemented — Phase 2, follow-up PR) will write `{"type":"session-summary","summary":"…","sessionId":"…","model":"…"}` into the session's JSONL at the end of each turn. The ingest path already handles this event type: `ingestLine` routes it into `session_summaries` and schedules it for embedding. Vectors are rebuilt from stored JSONL on DB drop + replay — the same idempotency guarantee as `events`. The `embeddings` and `session_summaries` tables (v6) are therefore safe to ship ahead of the producer.

**Local embeddings.** Turn text and session summaries are embedded by `bge-small-en-v1.5` (384-d, L2-normalized) running locally via the `@huggingface/transformers` library on the **native `onnxruntime-node`** backend (ONNX Runtime, not the WASM fallback). `onnxruntime-node` is a cross-build native module: prebuilts are fetched by `install-app-deps` for the target Electron ABI; it is declared in `asarUnpack` alongside `better-sqlite3`/`keytar`/`node-pty`. The transitive `sharp` dependency (present only in the `@huggingface/transformers` image-pipeline path) is excluded from packaging — it pulls in a large prebuilt and is not needed for text embeddings. Indexing failures degrade silently (per-turn errors logged, search never blocks ingest).
...
- **Schema docs**: the typed tools' descriptions name their columns. The `query` tool sees five tables — `events`, `sessions`, `broker_sessions`, `embeddings`, `session_summaries` — pre-filtered to the caller's allowed-workspace rows; `raw_jsonl` on `events` is opt-in via `include_raw`.
