...
- **Chip markers** (`WorkspaceTabStrip.tsx`, glyphs + role predicates in `committee.tsx`): a **wifi glyph** (info-blue) when reachable, a **hub-and-spokes "manager" glyph** (neutral `--ink`) when the workspace holds grants — both left of the status dot so opt-in/role is never silent. `isManager`/`isReachable` mirror the main-side `control.ts:isManager`.
