...
**Native modules.** `better-sqlite3`, `keytar` (retained for the legacy-purge migration), and `node-pty` ship as native bindings — they must match Electron's bundled Node ABI, not the system Node. The repo's `postinstall` script runs `electron-builder install-app-deps` to pull prebuilt binaries (or rebuild) for the current Electron version. Without this hook, `npm install` builds the bindings against the system Node and Electron fails to load them at runtime with a `NODE_MODULE_VERSION` mismatch. (The vault itself no longer needs a native module — `safeStorage` is built into Electron.) **`electron-builder.yml` must `asarUnpack` these native modules** (`**/*.node` plus the `better-sqlite3`/`keytar`/`node-pty` trees): a `.node` binary cannot be `dlopen`'d from inside `app.asar`, so without this the packaged app crashes the moment it touches SQLite, the keychain, or a PTY. electron-builder's automatic smart-unpack does **not** reliably catch all three (it missed `better-sqlite3` and `keytar` while unpacking `node-pty`), so the unpack list is declared explicitly.
...
**CI build matrix.** `.github/workflows/build-app.yml` builds the app on **native runners** (`macos-latest` → `.dmg`, `windows-latest` → `.exe`, `ubuntu-latest` → `.AppImage`), so the Linux→Windows cross-build workarounds above are *not* needed there — each platform compiles or fetches its own native-module binaries directly. It runs on PRs (build + upload artifacts) and on `v*` tags (build, then publish a **draft** GitHub Release with the unsigned installers attached). The Makefile remains the path for a local Windows build from a Linux/WSL dev box without CI.
...
**Run layer — reload (the "in-place `--resume`").** A loadout's files/merges land in the workspace's `.claude/`, but claude only reads its config **at session start** — so a freshly-installed loadout has no effect on a *running* session until claude restarts. Rather than make the user restart (the old destructive `stop`/`start` path loses the conversation and isn't reused here), the reload **closes and re-resumes the active session in place**: ① resolve the active tab's Claude session UUID (`observability:summaryForBrokerSession`), ② `pty:closeSession` the live handle (broker CLOSE ⇒ claude exits, transcript flushed to `<uuid>.jsonl`, session dropped), ③ re-attach the **same** broker session id with `resumeOf=<uuid>` ⇒ the broker `CREATE`s `claude --resume <uuid>`, so the **same tab** picks the conversation back up — now with the loadout loaded. The renderer flow: `LibraryPane`→`onInstalled(workspaceId)`→`App` (only if **autoReloadLoadouts** is on **and** the workspace is a running container) sets a per-workspace `reloadRequest`; the matching `TerminalPane` holds it pending and fires only once the **active session is idle** (busy/idle from the title-glyph `activityDetector`), handing a `reloadTarget {sessionId, token}` to the active `TerminalSession`, which runs the close+resume against itself. If no resumable UUID exists (claude never started in that tab), it's a no-op — the files are already in place for the next start. The reload touches the broker session lifecycle and can't be exercised in mock/CI (needs a real `claude --resume`), so it's verified by live container test.
...
- **Caller identity = per-workspace socket (#117)**: there is one listener per workspace at `<userData>/mcp/<id>/mcp.sock`, bind-mounted into only that container. The host stamps every `tools/call` with the workspace id of the listener that accepted the connection — **identity is ambient from the mount, never asserted on the wire** (no `caller_id` arg, token, or env var to forge or steal). This is the security spine the cross-workspace committee permission model (#116/#118) builds on. **Local-workspace caveat:** a local (non-container) workspace has full host-FS access and could open any socket, so its derived identity is *not* unspoofable — only containers' is; `assertControl` (#118) refuses local workspaces.
- **Visibility = fleet-global by default, optionally scoped (#122)**: by default a `sessions`/`events` row from one workspace is queryable by the agent in another (the historical "sessions are global" behavior). Setting **`CLAUDE_FLEET_SCOPED_READS=1`** flips on **scoped reads**: each caller's read tools (`list_sessions`, `get_session`, `get_cost`, `list_events`) return only its **own + `read`-granted, opted-in** workspaces' rows (resolved via `assertControl`), and the raw `query` hatch is **disabled** (it can express cross-workspace joins/aggregates that can't be safely row-filtered). The flag defaults **off** for one release (no regression for existing skills), then flips. Implementation: `mcpServer.ts` computes the caller's allowed set per call from an injected resolver (`ipc.ts:allowedReadWorkspaces`) and hands it to the tools via `ToolCtx.allowedWorkspaces` (`null` ⇒ unrestricted); the §9 read-only SQL guard is unchanged.
...
- The runner image installs `socat` (it had no `socat`/`nc`) for the stdio↔socket bridge; `sh` (base image) drives the reconnect loop.
- `ensureWorkspaceClaudeJson` writes a **user-scope** `mcpServers` entry into the per-workspace `~/.claude.json`: a **reconnecting** bridge `{ "claude-fleet-state": { type: "stdio", command: "sh", args: ["-c", "while :; do socat - \"UNIX-CONNECT:/fleet/mcp/mcp.sock,forever,interval=1\"; sleep 1; done"] } }`. The loop (not a bare `socat`) is what keeps MCP alive across an app restart while a container is paused: `forever` retries the connect while the host server is mid-restart, and the `while` restarts socat after an established connection drops. Because the server is **stateless per-connection**, the reconnected socket serves tool calls without re-`initialize`. User-scope entries are trusted, so `claude` auto-connects with **no approval gate** (verified: `claude mcp list` → `✔ Connected`). **Ownership split:** the onboarding/account fields (`hasCompletedOnboarding`, `projects`) are seeded only when the file is absent (claude owns them after), but the `mcpServers["claude-fleet-state"]` entry is **app-managed infrastructure** and is **reconciled on every create** (every other key preserved byte-for-byte; malformed files left untouched). That's what upgrades a pre-#18 workspace's stale one-shot `socat` to the reconnecting bridge when its container is recreated. **Caveat:** both the dir bind and the command only apply at container *create* time — a workspace that merely pauses/stops/starts keeps its original wiring until removed and recreated.
...
- **Opt-in** (`WorkspaceForm.tsx`, edit mode only): a "Committee access" disclosure — Reachable toggle + role hint + accept-from + a plain-language consent warning ("granted managers can read this session, type into it, and pause it"). Saved into `accessibility` via `workspace:writeManifest`. Off ⇒ `accessibility` cleared.
...
- **`committee_post(id, message)`** (MCP) / `committee:post` (IPC) → `assertControl('post')` → deliver `text + '\r'` to the expert's live session, the same fire-and-forget path as a human keystroke; no new broker frame.
  ...
  - **Headless fallback:** if no renderer is attached (a truly headless expert), fall back to `Backend.committeePost` — a **transient** broker attach (`LIST` → guard **0** "not attached yet" / **>1** "single-tab expert only" → `ATTACH` with reattach-retry → `sendInput` → `DETACH`), holding the one-writer slot only for the keystroke. `post` returns `{ via: 'attached' | 'headless' }`. Mock acks without a broker; local throws (container-only).
...
- **`committee_status(id)`** (MCP) / `committee:status` (IPC) → `assertControl('read')` → `{ paused, busy, stalled, lastActiveAt }`. `paused` from the workspace state; `busy` is **host-computed** (below); `stalled` = busy past the turn timeout; `lastActiveAt` best-effort from the DB.
...
- **Host-enforced budget (`src/main/committeeRuns.ts`).** A looping/misbehaving manager can't talk past these — they run in main, keyed by manager id:
  - **Max posts per run** (`COMMITTEE_MAX_POSTS`, default 40) and a **per-run USD ceiling** (`COMMITTEE_USD_CEILING`, default $10, summed via the cost path across the manager's controlled experts). Checked in `committeePost` *before* delivering; on breach the host **force-pauses every expert the manager controls** (bypassing `assertControl` — the host is enforcing, not the manager) and throws.
  ...
  - All three are env-overridable. **v1 run model:** no explicit "convene" primitive yet, so a "run" = everything a manager has posted since the host started (counter resets on restart); #123's run-committee skill can add an explicit reset. *(Defaults chosen here — revisit with the console in #123.)*
...
- **`committee-manager`** — ships a **`run-committee` skill** teaching the convene → post → poll `status` → `collect` → synthesize → pause loop over the `committee_*` MCP tools, plus a `settings.json` pre-granting the fleet MCP server. Grants over each expert are still set by the human in the left-rail Committee matrix (#118).
...
**Legacy fleet-global reads narrowed (#122).** See the "In-container SQLite access via MCP" subsection above: the read tools can now be scoped to the caller + its read-granted targets behind the `CLAUDE_FLEET_SCOPED_READS` flag (default off this release, then flip). **Migration:** any skill relying on cross-workspace reads keeps working while the flag is off; when it flips on, such a skill must hold an explicit `read` grant over each workspace it reads (and that workspace must be reachable), and must stop using the raw `query` tool for cross-workspace data (the typed tools are the scoped path).

**Phased delivery.** #117 socket/identity ✓ → #118 model + UI ✓ → #119 pause/unpause ✓ → #120 post/collect ✓ → #121 busy-idle + status + runaway guards ✓ → #122 scoped reads behind a flag ✓ → #123 expert/manager loadouts + `[committee]` inbound toast ✓ (committee console + mailbox durability deferred).
