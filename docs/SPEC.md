...
- **What ships:** `AskUserQuestion` and `ExitPlanMode` — the "Claude is waiting on the user" moments that *do* occur and *are* in the transcript as `tool_use` events already ingested into `events`. `db.userPromptsForWorkspace` is a query over `events` (no new table, no migration): tool_use rows with `tool_name IN ('AskUserQuestion','ExitPlanMode')`, LEFT-JOINed to their `tool_result` (by `tool_use_id`) to derive a `resolved` flag, mapped to `kind` `'ask'`/`'plan'` with the summarized `tool_input` as a preview. Exposed via `userPrompts:list(workspaceId)`. The observability pane (workspace scope) renders a passive **Prompts** section — `kind` (ask/plan) + preview + a `done`/`waiting` status, pending rows flagged with a warn accent (read-only, no one-click allow buttons, by design). The **chip "needs-input"** affordance is driven off a `pendingPrompt` flag on the per-workspace summary (`hasPendingPromptForSession` — an unresolved AskUserQuestion/ExitPlanMode in a running workspace): a pulsing red pip + danger-toned status line. **Remaining:** per-workspace disable flag; cross-session aggregation of repeat prompts.
