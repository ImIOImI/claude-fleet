...
- **Body row** (3 columns):
  ...
  - **Observability pane** (right, ~320px): live view of the **active terminal tab**'s Claude session in the selected workspace. Shows session title (from `ai-title` event or first-user-message head), latest model, last-activity relative time, event count, prominent USD total, token totals (input / cache-create / cache-read / output), and top tools by call count. Empty state when the workspace has no transcript events yet. Per-tab resolution comes from the `broker_sessions` mapping table (§11 *Per-tab mapping*). `TerminalPane` bubbles its active tab id up via `onActiveTabChange(workspaceName, brokerSessionId, isFresh)`; App.tsx fetches `summaryForBrokerSession(workspaceName, activeTabId)` for the selected workspace and re-fetches on every push for that workspace. When the per-tab fetch returns null (no mapping yet), the renderer's workspace-summary fallback is conditional on `isFresh`: tabs **loaded from sessions.json** (`isFresh=false`) fall back to the workspace summary (pre-PR tabs and concurrent-attach skip cases probably have real claude activity the mapping hasn't caught up to); tabs **freshly added via `+` or close-last-auto-recreate** (`isFresh=true`) stay on the empty state — the user just opened a clean tab and inheriting the previous tab's numbers would be confusing.
...
Per-session cost and token counts derived from Claude transcript JSONL events. **Status: foundation + cost rollup + pane v1 + slot consumers + live push + per-model context window + per-tab mapping shipped; tab-state richness and subagent JSONLs remain.**
...
- Per-tab mapping (`broker_sessions` table, `pendingAttaches.ts`): each terminal tab's broker session id maps to a specific claude session UUID. Learned passively — `attachPty` records a pending attach (workspace, broker_session, timestamp), and `JsonlWatcher` emits `'new-session'` the first time a JSONL appears for a path; the IPC layer pairs them when *exactly one* pending attach is in flight for that workspace within a 30s window. Concurrent attaches (e.g., three tabs re-mounted simultaneously) skip the auto-mapping by design — wrong mapping is strictly worse than no mapping. Mapping persists in SQLite so re-attaching after an app restart (when broker still has claude alive and no new JSONL appears) still resolves to the right session. **Fallback semantics:** `summaryForBrokerSession` returns null when no mapping exists; the **renderer** decides whether to fall back to the workspace summary based on whether the tab was loaded from `sessions.json` (yes → fall back; covers pre-PR tabs + concurrent-attach skip cases) or freshly added via `+`/close-last-auto-recreate (no → empty pane; the user explicitly opened a new tab and inheriting the previous tab's data would surface the wrong numbers — the user-reported "new session shows the last session's info" regression that triggered this split). Unit-tested in `src/main/pendingAttaches.test.ts`; e2e regression in `tests/smoke.spec.ts` ("summaryForBrokerSession: returns null for an unmapped broker session").
