...
- **Visibility = fleet-global (for now)**: a `sessions`/`events` row from one workspace is still queryable by the agent in another, matching the "sessions are global" goal. Now that caller identity is real, narrowing reads to the caller + its read-granted targets is #122 (behind a release-long flag).
