...
**Stopped state.** A `stopped` workspace is **not** in the top strip and has no main-pane (the always-mounted panes are running/paused only, #21) — it lives in the workspace modal's Saved list. Restart is the Saved-row **Resume** (writes the manifest, then `workspace:start(id)` → `docker start`; on the next `workspace:list` it flips to `running`, rejoins the strip, and gets auto-selected; sessions mount fresh from `sessions.json`). (An earlier iteration, #17, surfaced a "Start" overlay on a stopped chip *in the strip*; #21 superseded it by moving stopped out of the strip entirely.) The "+ new tab" button is disabled while paused. **Caveat (PR1):** PTYs bound to the (frozen) container thaw correctly across a pause that happens *while the app is running*; across an app restart they're re-spawned and in-memory state is lost — the broker layer that preserves it is deferred (see §11).
