...
- **Discoverability is `acceptFrom`-gated, NOT grant-gated.** `decideRoster` mirrors `decideControl`'s container-only + no-manager-target + reachable rules, but **drops the outbound-grant check** (discovery needs no grant — that's the point) and **tightens the inbound opt-in**: the target's `acceptFrom` must be **non-empty AND name the caller**. So an expert with an **open/empty `acceptFrom`** is *controllable-if-granted* but **NOT roster-visible** — `reachable` alone never advertises an expert fleet-wide; the operator must explicitly name the manager in the expert's `acceptFrom` to make it discoverable. `acceptFrom` ≠ grant, so the **discover → ask-for-grant** flow still works (an expert lists the manager before any grant exists, with `grant.controllable: false`). Note the resulting asymmetry vs. control: empty `acceptFrom` permits control-when-granted but not discovery.
