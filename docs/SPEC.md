...
- `get_config()` — returns effective fleet tunables for this workspace: `{ summarizer: { model, minNewTurns, minIntervalS, windowChars } }`, resolved as app defaults overridden by the workspace's `env.plain` (`CF_SUMMARY_MODEL`, `CF_SUMMARY_MIN_NEW_TURNS`, `CF_SUMMARY_MIN_INTERVAL_S`, `CF_SUMMARY_WINDOW_CHARS`). Reflects what the host set at container create; manual in-container env changes are not visible until recreate.
...
- **Schema docs**: the typed tools' descriptions name their columns. The `query` tool sees seven tables — `events`, `sessions`, `broker_sessions`, `embeddings`, `session_summaries`, `session_tags`, `usage_events` — pre-filtered to the caller's allowed-workspace rows; `raw_jsonl` on `events` is opt-in via `include_raw`.
