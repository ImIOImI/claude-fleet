...
- **Top row** (`WorkspaceTabStrip`): app name, workspace chips (each with a deterministic hue ring + status dot + a small secondary line showing recent activity from the workspace's observability summary, e.g. "active 2m ago" or "idle 1h ago"), `+ New workspace`, daemon status pill, `MOCK MODE` chip when active, `Profiles…`. Only live workspaces (running/stopped) appear here; deleted workspaces are surfaced in the new-workspace modal's past list. Summary data is centralized in `App.tsx` (single polling effect for every live workspace) and distributed to the chip strip + observability pane + terminal pane via props.
- **Body row** (3 columns):
...
- **Context bar** in the main pane: a 3px hue band riding the top edge of the terminal, separated from the session tab strip by 8px of breathing room (so the active-tab underline and the band read as two distinct elements). Width fills `0..100%` driven by the active session's last-turn context usage (`input_tokens + cache_read_input_tokens + cache_creation_input_tokens` over an assumed 200K context window). When no observability data exists, fill defaults to 100% so the bar reads as a pure identity marker. The 80%-mark compaction tick is a planned addition once the right per-model context-limit lookup lands.
...
- **Slot consumers.** Chip secondary line and context-bar fill ship in #34; the per-tab status dot (#24) is a follow-up that builds the UI surface and wires it. Full chip redesign — color identity bar, 2-line content, terminal pips, needs-input variant — is the separate visual-fidelity work in #20.
