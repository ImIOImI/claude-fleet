...
- **Transcript ingestion.** A local workspace's Claude transcripts live in the host's `~/.claude/projects/<encodeClaudeProjectDir(workspaceRoot)>/`. The watcher registers that specific subdirectory (mapped to the real workspace id via `JsonlWatcher.hostDirs`) at startup and on `workspace:create`, and unregisters on `workspace:remove` — so local spend is ingested and attributed correctly while unrelated personal projects in the same `~/.claude/projects/` tree are never touched.
