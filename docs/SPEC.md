...
- **Sync is entirely host-side.** Both versions live on the host — the library's `loadout.md` and the workspace's host-private manifest — so "update available" is just `semver.gt(library, installed)` and **Update** (revert old snapshot → copy new) runs host-side. The container never participates in version sync and never sees the library. Updates are **explicit** (a reviewed action, since scripts re-run / files change), not automatic; a file the user edited after install is **kept and flagged** on update, never silently overwritten.
