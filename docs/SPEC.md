...
- `observability:summaryForBrokerSession(workspaceName, brokerSessionId)` → `WorkspaceSummary | null` — per-tab variant. Resolves the broker→claude mapping in the `broker_sessions` table and returns that claude session's summary; falls back to `summaryForWorkspace` when no mapping is known yet (concurrent-attach skip, mapping pre-dates the table, or per-tab fetch raced ahead of the watcher's new-session learn). Same shape as the workspace summary so the renderer treats both endpoints interchangeably.
...
- Per-tab mapping (`broker_sessions` table, `pendingAttaches.ts`): each terminal tab's broker session id maps to a specific claude session UUID. Learned passively — `attachPty` records a pending attach (workspace, broker_session, timestamp), and `JsonlWatcher` emits `'new-session'` the first time a JSONL appears for a path; the IPC layer pairs them when *exactly one* pending attach is in flight for that workspace within a 30s window. Concurrent attaches (e.g., three tabs re-mounted simultaneously) skip the auto-mapping by design — wrong mapping is strictly worse than no mapping; the renderer's fallback (workspace summary) keeps the UI usable in the ambiguous case. Mapping persists in SQLite so re-attaching after an app restart (when broker still has claude alive and no new JSONL appears) still resolves to the right session. **Known limitation:** tabs that pre-date this table never learn a mapping on re-attach — the broker still has claude alive, no new JSONL appears, and the watcher's `'new-session'` doesn't fire. They display the workspace summary fallback until they're recreated. New tabs and new workspaces map correctly from day one. Unit-tested in `src/main/pendingAttaches.test.ts`.
