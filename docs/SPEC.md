...
- `inputwait:update` (main → renderer push) — `{ workspaceId, waitingSessionIds: string[] }`. The set of claude session UUIDs in that workspace currently blocked on `AskUserQuestion`. Sent whenever the set changes (a session enters or leaves the waiting state) and cleared on workspace stop/remove. Exposed as `window.api.inputwait.onUpdate(cb)`.
...
- **Schema docs**: the typed tools' descriptions name their columns. The `query` tool sees three tables — `events`, `sessions`, `broker_sessions` — pre-filtered to the caller's allowed-workspace rows; `raw_jsonl` on `events` is opt-in via `include_raw`.
