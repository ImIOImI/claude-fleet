...
1. **Debounce — completed turns, not lines or raw prompts.** Transcript lines are a bad novelty proxy (one tool-heavy turn emits dozens), and raw prompt counts can trigger mid-way through a rapid copy/paste burst. Count **completed turns** — a typed human prompt (string content in claude JSONL; tool results carry content *arrays*) followed by an assistant reply, which is exactly what each `Stop` firing marks — against `<uuid>.fleet.state`. Re-summarize only when **≥ `CF_SUMMARY_MIN_NEW_TURNS` (default 20)** new completed turns have landed since the last summary **and ≥ `CF_SUMMARY_MIN_INTERVAL_S` (default 120)** seconds have passed since the last summarizer run. Cost is bounded by conversation pace, invariant to tool storms and paste bursts. Most Stop firings cost one jq pass.

**Tunables — surfaced, not buried.** All knobs are env vars read by the hook and therefore settable **per workspace through the app's existing workspace-env feature** (no new UI): `CF_SUMMARY_MIN_NEW_TURNS` (20), `CF_SUMMARY_MIN_INTERVAL_S` (120), `CF_SUMMARY_MODEL` (haiku), `CF_SUMMARY_WINDOW_CHARS` (8000). A new read-only MCP tool **`get_config`** returns the effective summarizer settings (app defaults ⊕ the workspace's env overrides) so agents and users can inspect the knobs without spelunking scripts — with the noted caveat that the host reports what it set at create; manual in-container env mutation lags until recreate. Extensible home for future tunables.
...
**Known risk (resolve during implementation):** the `claude -p` run writes its own throwaway transcript. Run it from `/tmp` and verify the watcher doesn't index it; fallback: filter at ingest by cwd. Cost profile: ≤ one haiku call per 20 completed turns and ≥120s apart, on the workspace's own auth.
...
- New tools: `report_session_mapping`, `mark_useful` (write-via-injected-handler, caller-scoped), `get_config` (read-only; effective summarizer tunables for the caller's workspace).
