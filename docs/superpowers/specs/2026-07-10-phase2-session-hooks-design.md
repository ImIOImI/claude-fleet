...
1. **Debounce:** compare the transcript's line count against `<uuid>.fleet.state`; exit unless ≥ `CF_SUMMARY_MIN_NEW_LINES` (default 20) new lines since the last summary. Most turns cost one `wc -l`.
